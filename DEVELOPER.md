...
- 引入线性压缩映射：`compressionRatio = min(1, 6M / totalSize)`，spacer 和 scrollTop 统一压缩
- `MAX_SAFE_SCROLL_UNSCALED = 6,000,000px`，即使在最大 5x zoom 下物理总量 = 30M < 33.5M
- 当 totalSize ≤ 6M 时 ratio=1（恒等映射，零开销）
- 所有读取 scrollTop 的位置做解压缩：`logicalOffset = scrollPos / zoom / ratio`
- 所有写入 scrollTop 的位置做压缩：`scrollTop = logicalOffset * ratio * zoom`
...
**修复**: 引入虚拟坐标线性压缩。`compressionRatio = min(1, 6M / totalSize)`，所有 spacer 和 scrollTop 统一乘以 ratio。6M 在 5x zoom 下 = 30M，低于 Chromium 33.5M 上限。totalSize ≤ 6M 时 ratio=1（恒等映射）。所有读取 scrollTop 的位置做 `/ ratio` 解压缩，所有写入 scrollTop 的位置做 `* ratio` 压缩。
...
**修复**: 滚动条比例改用逻辑坐标计算：`logicalPos = scrollPos / zoom / ratio`，比值 = `logicalPos / totalSize`。由于压缩比和 zoom 在公式中相消，滚动条位置完全由"当前在第几张图"决定，与缩放级别无关。
