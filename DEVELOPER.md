...
- ZoomConfig: 缩放限制配置
- MAX_SAFE_SCROLL_UNSCALED: Chromium scrollHeight 安全上限 (6M px, 在 imageLoader.js 中)
```
...
- `getTotalSize()`: 获取逻辑总大小（未压缩的前缀和总量）
- `getCompressionRatio()`: 获取虚拟坐标压缩比（totalSize ≤ 6M 时返回 1）
- `forceRenderVisibleItems()`: 供外部模块强制刷新虚拟滚动 spacer
```
...
**虚拟坐标压缩 (v1.5.2) - 突破 Chromium scrollHeight 上限：**
- Chromium 的 `scrollHeight` 硬上限约 33,554,432px (2^25)，10万+图片的逻辑总高度轻松超过 1 亿 px
- 引入线性压缩映射：`compressionRatio = min(1, 6M / totalSize)`，spacer 压缩，内容区 1:1
- `MAX_SAFE_SCROLL_UNSCALED = 6,000,000px`，即使在最大 5x zoom 下物理总量 = 30M < 33.5M
- 当 totalSize ≤ 6M 时 ratio=1（恒等映射，零开销）
- 滚动条比例用逻辑坐标计算，不依赖 volatile 的 `scrollHeight`

**分段坐标映射 (v1.5.3) - 压缩空间精确定位：**

虚拟滚动的物理滚动空间由三段组成，各段映射斜率不同：
```
spacerTop（压缩 ratio*zoom） | 真实内容（1:1 * zoom） | spacerBottom（压缩 ratio*zoom）
```
均匀公式 `scrollPos / zoom / ratio` 在内容区域是**错的**（内容未压缩），必须分段处理：
- `physicalToLogicalRaw(scrollPos, rangeStart, rangeEnd)` — 物理 → 逻辑
- `logicalToPhysicalRaw(logicalOffset, rangeStart, rangeEnd)` — 逻辑 → 物理
- 导出的 `physicalToLogical()` / `logicalToPhysical()` 使用当前 `renderedRange`

**踩坑记录 — 分段映射引入后必须注意的陷阱：**

1. **renderVisibleItems 的 scrollTop 补偿方向**：spacer 变更后 `physicalAfter - scrollPos` 补偿的锚点是**视口顶部/左端**。如果调用方的锚点不在视口顶部（如鼠标锚定缩放），必须在 `forceRenderVisibleItems()` 之后**重新用分段公式定位锚点**。
2. **jumpToPage / reloadForModeSwitch 的两步定位**：先用均匀公式（`renderedRange={-1,-1}` 全压缩状态）粗定位 → `renderVisibleItems()` 建立 renderedRange → 再用分段公式精确修正。不能省略第二步。
3. **RTL 横屏模式的坐标轴翻转**：`Math.abs(scrollLeft)` 和 `physicalToLogical` 都从内容**起始端**量起，但 `mouseInViewportX` 从视口**左边缘**量起。RTL 下内容起始端在右侧，鼠标偏移必须翻转为 `clientWidth - mouseInViewportX`。
4. **拖拽与缩放的 scrollTop 争夺**：左键按下同时激活 `isDragging=true`（drag.js）和缩放检测（zoom.js）。鼠标微动触发 drag 的 mousemove handler 覆盖缩放设的 scrollTop → 跳变。解法：缩放期间 `setDragSuppressed(true)`，结束后重置拖拽基准再恢复。
5. **滚轮速度归一化**：压缩激活时浏览器默认 deltaY 在物理空间中对应的逻辑距离被放大（1px 物理 = 1/ratio px 逻辑），需要拦截 wheel 事件并用分段映射转换为固定逻辑步长（~300px/tick）。
...
- adjustDragBaseline(): 虚拟滚动 spacer 变更后调整拖拽基准值
- setDragSuppressed(): 缩放期间抑制拖拽 mousemove，防止覆盖缩放设的 scrollTop
```
...
**Bug: 跳转 >26000 页全部卡在 ~26000（10万+图片时）**

**现象**: 打开 10万张图片，输入页码 5000/10000 可准确跳转，但输入 >26000 的数字全部跳到 ~26000 位置。

**根因**: Chromium 对 `scrollHeight` 有硬上限约 33,554,432px (2^25)。10万张图片的逻辑总高度 ≈ 1.2亿 px，远超上限。无论 spacer 设多大，浏览器都会 clamp `scrollTop` 到 `scrollHeight - clientHeight`，而 26000 页 × 1200px ≈ 3120 万 px 已接近上限。

**修复**: 引入虚拟坐标线性压缩。`compressionRatio = min(1, 6M / totalSize)`，spacer 和 scrollTop 统一压缩。后续在 v1.5.3 中发现均匀压缩公式在内容区域是错的（内容未压缩），升级为分段映射。

**Bug: 缩放导致滚动条位置大幅跳动**

**现象**: 在第 5000 张图位置缩放，滚动条跳到底部（放大到最大时）或其他错误位置。缩小回来后滚动条恢复。页码也跟着跳变。

**根因**: `updateVerticalScrollbar()` 使用 `viewport.scrollHeight`（volatile 值，取决于当前 spacer 覆盖范围）计算滚动条比例。缩放改变布局尺寸后，spacer 可能不覆盖完整范围，`scrollHeight` 比实际逻辑总量小得多，导致 `scrollPos / (scrollHeight - clientHeight)` 比例偏大。

**修复**: 滚动条比例改用逻辑坐标计算：`logicalPos = physicalToLogical(scrollPos)`，比值 = `logicalPos / totalSize`。
...
### v1.5.3
**Bug 批量修复：分段坐标映射 + 交互冲突（10万+图片）**

均匀压缩公式 `scrollPos / zoom / ratio` 在内容区域是错的（spacer 被压缩但内容是 1:1 真实尺寸），导致大数据量下一系列定位偏移。

**Bug 1: 跳转页码不准确** — 输入 50000 跳到 49995 或更远。
- 根因：`jumpToPage` 用均匀公式定位，但 `renderVisibleItems` 后内容区出现（1:1 映射），视口中心逻辑位置漂移。
- 修复：两步定位——均匀粗定位 → renderVisibleItems → 分段公式精确修正。

**Bug 2: 左键+滚轮缩放画面跳变** — 按住左键滚轮缩放时画面不稳定。三个独立根因：
- (a) `physicalToLogical(scrollPos + mouseOffset)` 的分段映射在区域边界不是线性的，不能用 `logicalAtTop + mouseOffset/zoom` 线性化。修复：直接用 `physicalToLogical(mainAxisScroll + mousePhysicalOffset)`。
- (b) `forceRenderVisibleItems()` 内部的 scrollTop 补偿按视口顶部保持，但鼠标锚点缩放的锚点在鼠标位置。当两者落在分段映射的不同区域（不同斜率），补偿拉偏鼠标锚点。修复：render 后重新用分段公式定位鼠标锚点。
- (c) 左键按下同时激活 drag.js 的 `isDragging`，鼠标滚轮时不可避免的微小位移触发 drag 的 mousemove handler 覆盖缩放设的 scrollTop。修复：缩放期间 `setDragSuppressed(true)`，结束后重置拖拽基准再恢复。

**Bug 3: RTL 横屏模式鼠标锚定缩放失效** — 退化为 Ctrl+wheel 效果。
- 根因：`mouseInViewportX` 从视口左边缘量起，但 RTL 下内容起始端在右侧。鼠标偏移未翻转为 `clientWidth - mouseInViewportX`，导致锚点镜像到错误位置。
- 修复：在读取和写入两侧都加 RTL 翻转。

**Bug 4: 模式切换后页码偏移几页** — 竖向切横向或反向后当前页码前移。
- 根因：`reloadForModeSwitch` 用均匀公式定位后 `renderVisibleItems` 建立了分段映射，但未做精确修正。
- 修复：同 Bug 1，renderVisibleItems 后用分段公式重新居中。

**Bug 5: 滚轮滚动速度过快（10万+图片时）** — 一滚跳过几十页。
- 根因：压缩激活时 1px 物理 = 1/ratio px 逻辑，浏览器默认 deltaY 在逻辑空间被放大。
- 修复：拦截 wheel 事件，用分段映射转换为固定逻辑步长 ~300px/tick。
